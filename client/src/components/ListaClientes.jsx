...
const ListaClientes = ({openModalEditar, openModalEliminar, clientesCounter}) => {
  ...
  const handleAssignComprobante = async (comprobante) => {
    ...
    const res = await fetch(`/api/clientes/${selectedCliente.id}/comprobantes`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
        ...
        <div>
          ...
          <ul>
            {comprobantes.map(comprobante => (
              <li key={comprobante.id} onClick={() => handleAssignComprobante(comprobante)}>
          ...
          <ul>
            {assignedComprobantes.map(comprobante => (
              <li key={comprobante.id}>
                {comprobante.Fecha} - ${comprobante.TotalVenta} - {comprobante.estado}
                {comprobante.estado === 'pagado' && ` - Pagado el ${new Date(comprobante.fechaPago).toLocaleDateString()}`}
              </li>
            ))}
          </ul>
        </div>
