      ...
      <div className="flex flex-col items-center justify-between bg-slate-200 m-auto max-w-sm rounded-md">
        ...
        <form className="flex flex-col items-center gap-2 w-[100%] p-4">
          ...
          <div className="flex flex-col items-start">
           ...
           {isPasswordMatch ? <span className="text-red-500 text-sm">Password no coinciden!</span> : "" } 
           {isPasswordShort ? <span className="text-red-500 text-sm">Debe Tener al menos 8 caracteres</span> : "" } 
          </div>

          <div className="p-2">
            <button onClick={handleSubmit} className="bg-blue text-white font-semibold p-2 rounded">Register</button>
          </div>
        </form>
        <div className="flex mb-3">
          <p>¿Ya tenes cuenta? <Link to="/login" className="text-blue font-semibold">Iniciar Sesión</Link></p>
        </div>
      </div>
    </div>
  );
